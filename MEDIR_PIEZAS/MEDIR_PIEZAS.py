...
def measure(img):
    ...
    for cnt in contours:	 # Encuentra la distancia geométrica del contorno
        M = cv.moments(cnt)
...
while(cap.isOpened()):
    ret, frame = cap.read()
    img = cv2.flip(frame, -1) #Cree una ventana GUI en forma de adaptación
    cv.namedWindow("input image", cv2.WINDOW_AUTOSIZE) 	 #Conecta la imagen a la ventana por nombre
    cv.imshow("input image", img)
    measure(img)	 # Si presiona la tecla P, la imagen se guardará en D: / Archivos de programa / kk.jpg y saldr
